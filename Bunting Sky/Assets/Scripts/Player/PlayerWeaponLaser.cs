...
public class PlayerWeaponLaser : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            GameObject instancePlayerLaserProjectile = Instantiate(playerLaserPrefab, Vector3.zero, Quaternion.identity);
            POOL.Add(instancePlayerLaserProjectile);
            instancePlayerLaserProjectile.SetActive(false);

            //Put in weapons tree
            instancePlayerLaserProjectile.transform.parent = player.playerWeaponsTreeLaser.transform;

            //Pass control reference
            instancePlayerLaserProjectile.GetComponent<PlayerWeaponLaserProjectile>().control = control;
        }
    }
    ...
    public void FireWeaponLaser()
    {
        ...
        POOL[poolIndex].GetComponent<PlayerWeaponLaserProjectile>().timeAtWhichThisSelfDestructs = PROJECTILE_LIFETIME_DURATION;
        POOL[poolIndex].GetComponent<PlayerWeaponLaserProjectile>().timeSpentAlive = 0f;
